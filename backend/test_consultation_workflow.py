...
def test_consultation_owned_by_doctor_and_completes():
    ...
    r = client.post('/api/encounters', headers=patient, json={'patient_id':1,'doctor_id':demo_doctor_id(),'department':'AI5D-Test','priority':'normal','reason':'Consultation test'})
    assert r.status_code == 200, r.text
...
def test_completed_consultation_is_locked():
    ...
    if cid is None:
        patient = auth(client, 'patient@sih26047.local', 'patient123')
        r = client.post('/api/encounters', headers=patient, json={'patient_id':1,'doctor_id':demo_doctor_id(),'department':'AI5D-Lock','priority':'normal','reason':'Lock test'})
        eid = r.json()['encounter']['id']
