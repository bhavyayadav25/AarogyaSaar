...
from clinical_safety_triage import evaluate_safety, PRIORITY
...
def build_risk_assessment(patient: Any, consultations: Iterable[Any], documents: Iterable[Any], summary: Dict[str, Any] | None = None) -> Dict[str, Any]:
    ...
    # Preserve the explicit severity reported during intake when available.
    severity_level = safety.get("severity_level", "not_assessed")
    assessment_status = "complete" if consultations or any(_doc_items(d) for d in documents) else "incomplete"
    return {
