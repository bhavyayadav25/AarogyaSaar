...
def test_encounter_queue_flow():
    ...
    r = client.post('/api/encounters', headers=patient_headers, json={
        'patient_id': 1, 'doctor_id': demo_doctor_id(), 'department': 'General Medicine', 'priority': 'normal', 'reason': 'Routine visit'
    })
    assert r.status_code == 200, r.text
...
def test_invalid_transition_rejected():
    ...
    r = client.post('/api/encounters', headers=patient_headers, json={
        'patient_id': 1, 'doctor_id': demo_doctor_id(), 'department': 'ENT', 'priority': 'normal', 'reason': 'Transition test'
    })
    assert r.status_code == 200, r.text
...
def test_duplicate_active_encounter_rejected():
    ...
    client = TestClient(main.app)
    headers = auth(client, 'patient@sih26047.local', 'patient123')
    # Use the seeded department so the clean demo fixture remains valid.
    r = client.post('/api/encounters', headers=headers, json={
        'patient_id': 1, 'department': 'General Medicine', 'priority': 'normal', 'reason': 'Duplicate test'
    })
    assert r.status_code == 200, r.text
    r = client.post('/api/encounters', headers=headers, json={
        'patient_id': 1, 'department': 'General Medicine', 'priority': 'normal', 'reason': 'Duplicate test 2'
    })
    assert r.status_code == 409
