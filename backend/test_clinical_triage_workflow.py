...
def demo_doctor_id():
    ...
    try:
        return db.query(main.User.id).filter(main.User.email == "doctor@sih26047.local").scalar()
    finally:
        db.close()
...
def test_triage_dashboard_is_role_protected_and_operational():
    ...
    r = client.post('/api/encounters', headers=patient, json={'patient_id': 1, 'doctor_id': demo_doctor_id(), 'department': 'AI5E-Test', 'priority': 'urgent', 'reason': 'Triage test'})
    assert r.status_code == 200, r.text
...
def test_triage_actions_are_human_and_audited():
    ...
    r = client.post('/api/encounters', headers=patient, json={'patient_id': 1, 'doctor_id': demo_doctor_id(), 'department': 'AI5E-Action', 'priority': 'normal', 'reason': 'Action test'})
    assert r.status_code == 200, r.text
    eid = r.json()['encounter']['id']
...
def test_triage_cannot_act_on_closed_encounter():
    ...
    patient = auth(client, 'patient@sih26047.local', 'patient123')
    triage = auth(client, 'doctor@sih26047.local', 'doctor123')
    r = client.post('/api/encounters', headers=patient, json={'patient_id': 1, 'doctor_id': demo_doctor_id(), 'department': 'AI5E-Closed', 'priority': 'normal', 'reason': 'Closed test'})
    assert r.status_code == 200, r.text
    eid = r.json()['encounter']['id']
